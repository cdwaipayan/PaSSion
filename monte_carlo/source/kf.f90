...
!====================================================================================================
    ...
    ! The well-depth of patch A is used to define the reduced unit
    ! of temperature and so KFAA=1 for all cases.
    ! "Gyroidal" particles are 4-patch particles with patch-pairs 
    ! centered on each hemisphere.
    !----------------------------------------------------------------
    
        USE COMMONS, ONLY: DP, PI, NSITES, REFSITE, KFIJ, KFLAM2, KFDEL
        USE COMMONS, ONLY: KFAA, KFBB, KFCC, KFDD, KFLAMA, KFLAMB, KFLAMC, KFLAMD, KFDELA, KFDELB, KFDELC, KFDELD
        USE COMMONS, ONLY: GYBETA1, GYBETA2, SKEWAB
    
        IMPLICIT NONE

        REAL(KIND=DP) :: SS, CS, RMZ(3,3), HGYBETA1RAD, HGYBETA2RAD
        ...
        KFLAM2(1,1) = KFLAMA**2
        !HALF BETA (RADIANS)
        HGYBETA1RAD = GYBETA1*0.5_dp*PI/180_dp
        HGYBETA2RAD = GYBETA2*0.5_dp*PI/180_dp
        
        !INITIAL REFSITES (WITH BETA)
        REFSITE(:,1)= [  0.0_dp,  SIN(HGYBETA1RAD),  -COS(HGYBETA1RAD) ]
        REFSITE(:,2)= [  0.0_dp, -SIN(HGYBETA1RAD),  -COS(HGYBETA1RAD) ]
        REFSITE(:,3)= [  0.0_dp,  SIN(HGYBETA2RAD),   COS(HGYBETA2RAD) ]
        REFSITE(:,4)= [  0.0_dp, -SIN(HGYBETA2RAD),   COS(HGYBETA2RAD) ]
        ...
        !APPLY CHI TO THE 2ND PAIR
        REFSITE(:,3) = MATMUL(RMZ, REFSITE(:,3))
        REFSITE(:,4) = MATMUL(RMZ, REFSITE(:,4))
