SUBROUTINE GENERATE_UNIT_CELLS()
    ...
    USE COMMONS, ONLY: DP, NPART, NSITES, LID, VLM, RIGIDT, EQUISEEDT, RANDQUATT, SETORTN, NLAYERS, ANYCELLST, BNRYA
    USE COMMONS, ONLY: R, Q, BOX, NUCX, NUCY, NUCZ, ORTHO_IN, DENSITYT, PACKINGT, RHO, RHOX, RHOF, PI, BYUKWAT
    USE COMMONS, ONLY: SPHERERAD, SPHERECNFT, OBJSURFT
    USE ROTATIONS_MODULE, ONLY: RANDOM_QUATERNION

    IMPLICIT NONE

    INTEGER                     :: J1, I, IX, IY, IZ, M, IREF, NPCELL, NCOUNT
...
!------------------------------------------------------------------------------------
    ...
    ELSE IF (LID == 30 .OR. LID == 31) THEN
        IF(LID==30) NPCELL = 4
        IF(LID==31) NPCELL = 6
        CALL HONEYCOMB(A1,A2,A3)
!====================================================================================
!====================================================================================
!------------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------------
    ...
    IF(NPCELL*NUCX*NUCY*NUCZ /= NPART .AND. (.NOT. EQUISEEDT) .AND. (.NOT. ANYCELLST) .AND. (.NOT. BYUKWAT)) THEN
    ...
    M = 0
    IF(ANYCELLST) NCOUNT = 0
    DO IZ = 1, NUCZ
        DO IY = 1, NUCY
            DO IX = 1, NUCX
                DO IREF = 1, NPCELL
                    IF(ANYCELLST .AND. NCOUNT >= NPART) CYCLE
...
!     Shift the centre of the box to the origin
        DO I = 1, NPART
            IF(OBJSURFT) THEN
                R(1:2,I) = R(1:2,I) - BOX(1:2) / 2.0_dp
                R(1:2,I) = R(1:2,I) - ANINT(R(1:2,I)/BOX(1:2))*BOX(1:2)
            ELSE
                R(:,I) = R(:,I) - BOX / 2.0_dp
                R(:,I) = R(:,I) - ANINT(R(:,I)/BOX)*BOX
            ENDIF
...
!     This subroutine constructs a simple cubic lattice
    ...
    USE COMMONS, ONLY: DP, R, BOX, RHO, NUCX, NUCY, NUCZ, DENSITYT, PI, ANYCELLST, VLM, NPART
...
SUBROUTINE PEROVSKITE(UCL)
    ...
    IF (DENSITYT) THEN
        UCL = (3.0_dp/RHO)**(1.0_dp/3.0_dp)
    ELSE
        UCL = (3.0_dp*PI/(6.0_dp*RHO))**(1.0_dp/3.0_dp)
    ENDIF
...
!   Build the unit cell
!   Particles sitting on cubic tetrastack lattice points
    R(:,1)  = [UCL1, 0.0_dp, 0.0_dp]
    R(:,2)  = [0.0_dp, UCL1, 0.0_dp]
    R(:,3)  = [0.0_dp, 0.0_dp, UCL1]
...
SUBROUTINE KAGOME(A1, A2, A3)
    ...
    IMPLICIT NONE
    REAL(KIND=DP) :: IDEAL_CELL_VLM, CELL_VLM, A1, A2, A3, CN1, CN3, TWOR2
    ...
    A1  = 2.0_dp*(CELL_VLM/IDEAL_CELL_VLM)**(1.0_dp/2.0_dp)!3.1_dp
    A2  = SQRT(3.0_dp)*A1
    A3  = 3.0_dp
    BOX = [ A1*REAL(NUCX,DP), A2*REAL(NUCY,DP), A3*REAL(NUCZ,DP) ]
    PRINT *, "KAGOME UNIT CELL:", A1, BOX
...
SUBROUTINE HEXAGONAL(A1, A2, A3)
    ...
    IDEAL_CELL_VLM = 2.0_dp*SQRT(3.0_dp)
    
    A1  = 2.0_dp*(CELL_VLM/IDEAL_CELL_VLM)**(1.0_dp/2.0_dp)!3.1_dp
    A2  = SQRT(3.0_dp)*A1 / 2.0_dp
    A3  = 5.0_dp!*(CELL_VLM/IDEAL_CELL_VLM)
    BOX = [ A1*REAL(NUCX,DP), A2*REAL(NUCY,DP), A3*REAL(NUCZ,DP) ]
    PRINT *, "HEXAGONAL UNIT CELL:", A1, BOX

    R(:,1) = [0.0_dp, 0.0_dp, 0.5_dp]
    R(:,2) = [A1*(1.0_dp/2.0_dp),        0.0_dp,      0.5_dp]
    R(:,3) = [A1*(1.0_dp/4.0_dp), A2*(1.0_dp/2.0_dp), 0.5_dp]
    R(:,4) = [A1*(3.0_dp/4.0_dp), A2*(1.0_dp/2.0_dp), 0.5_dp]

    IF(RIGIDT) THEN
        Q(:,1) = UV_TO_Q ( [0.0_dp,   0.0_dp,   -1.0_dp] )
        Q(:,2) = UV_TO_Q ( [0.0_dp,   0.0_dp,   -1.0_dp] )
...
SUBROUTINE HONEYCOMB(A1, A2, A3)
    USE COMMONS, ONLY: DP, R, Q, RIGIDT, BOX, RHO, NUCX, NUCY, NUCZ, DENSITYT, PI, LID
    USE ROTATIONS_MODULE, ONLY: UV_TO_Q, QUATMUL
    IMPLICIT NONE
    INTEGER       :: J1
    REAL(KIND=DP) :: IDEAL_CELL_VLM, CELL_VLM, A1, A2, A3, CN1, CN3, Q1(4),Q2(4),Q3(4),Q4(4),Q5(4),Q6(4)
...
!   Unit cell length
    ...
    ELSE
        CELL_VLM = PI*4.0_dp / (6.0_dp * RHO)
    ENDIF
    IDEAL_CELL_VLM = 3.0_dp*SQRT(3.0_dp)
    
    A1  = SQRT(3.0_dp)*(CELL_VLM/IDEAL_CELL_VLM)**(1.0_dp/2.0_dp)!+0.08_dp
    A2  = 3.0_dp*(CELL_VLM/IDEAL_CELL_VLM)**(1.0_dp/2.0_dp)
    A3  = 2.0_dp*(CELL_VLM/IDEAL_CELL_VLM)**(1.0_dp/2.0_dp)
    
    BOX = [ A1*REAL(NUCX,DP), A2*REAL(NUCY,DP), A3*REAL(NUCZ,DP) ]
    IF(LID==30) THEN
        PRINT *, "HONEYCOMB UNIT CELL:", BOX
    ELSE
        PRINT *, "TRIANGULAR UNIT CELL:", BOX
    ENDIF
    
    R(:,1) = [0.0_dp, 0.0_dp, 0.5_dp]
    R(:,2) = [A1*(1.0_dp/2.0_dp), A2*(1.0_dp/6.0_dp), 0.5_dp]
    R(:,4) = [0.0_dp,             A2*(2.0_dp/3.0_dp), 0.5_dp]
    R(:,3) = [A1*(1.0_dp/2.0_dp), A2*(1.0_dp/2.0_dp), 0.5_dp]
    IF(LID==31) THEN
        R(:,5) = [A1*(1.0_dp/2.0_dp), A2*(5.0_dp/6.0_dp), 0.5_dp]
        R(:,6) = [0.0_dp,             A2*(1.0_dp/3.0_dp), 0.5_dp]
    ENDIF
    IF(RIGIDT) THEN
        DO J1 = 1, 4
...
SUBROUTINE FDDD(A1, A2, A3)
    ...
    IF(NSITES==2) THEN
        ...
        IF(RHO<0.5_dp) THEN
            ...
            R(:,1)  = [A1*0.125_dp, A2*0.125_dp, A3*0.125_dp];R(:,2)  = [A1*0.125_dp, A2*0.875_dp, A3*0.875_dp]
            R(:,3)  = [A1*0.000_dp, A2*0.000_dp, A3*0.000_dp];R(:,4)  = [A1*0.875_dp, A2*0.875_dp, A3*0.125_dp]
            R(:,5)  = [A1*0.750_dp, A2*0.750_dp, A3*0.250_dp];R(:,6)  = [A1*0.875_dp, A2*0.125_dp, A3*0.875_dp]
            R(:,7)  = [A1*0.125_dp, A2*0.625_dp, A3*0.625_dp];R(:,8)  = [A1*0.125_dp, A2*0.375_dp, A3*0.375_dp]
            R(:,9)  = [A1*0.000_dp, A2*0.500_dp, A3*0.500_dp];R(:,10) = [A1*0.875_dp, A2*0.375_dp, A3*0.625_dp]
            R(:,11) = [A1*0.750_dp, A2*0.250_dp, A3*0.750_dp];R(:,12) = [A1*0.875_dp, A2*0.625_dp, A3*0.375_dp]
            R(:,13) = [A1*0.625_dp, A2*0.125_dp, A3*0.625_dp];R(:,14) = [A1*0.625_dp, A2*0.875_dp, A3*0.375_dp]
            R(:,15) = [A1*0.500_dp, A2*0.000_dp, A3*0.500_dp];R(:,16) = [A1*0.375_dp, A2*0.875_dp, A3*0.625_dp]
            R(:,17) = [A1*0.250_dp, A2*0.750_dp, A3*0.750_dp];R(:,18) = [A1*0.375_dp, A2*0.125_dp, A3*0.375_dp]
            R(:,19) = [A1*0.625_dp, A2*0.625_dp, A3*0.125_dp];R(:,20) = [A1*0.625_dp, A2*0.375_dp, A3*0.875_dp]
            R(:,21) = [A1*0.500_dp, A2*0.500_dp, A3*0.000_dp];R(:,22) = [A1*0.375_dp, A2*0.375_dp, A3*0.125_dp]
            R(:,23) = [A1*0.250_dp, A2*0.250_dp, A3*0.250_dp];R(:,24) = [A1*0.375_dp, A2*0.625_dp, A3*0.875_dp]
            IF(RIGIDT) THEN
                CN1 = 0.43922692270767_dp
                CN2 = 0.74637770845439_dp
                CN3 = 0.5_dp
                Q(:,1)  = UV_TO_Q ( [ CN1,     CN2,    0.5_dp] );Q(:,2)  = UV_TO_Q ( [-CN1, CN2,0.5_dp] )
                Q(:,3)  = UV_TO_Q ( [ 0.0_dp, 0.0_dp, -1.0_dp] );Q(:,4)  = UV_TO_Q ( [-CN1,-CN2,0.5_dp] )
                Q(:,5)  = UV_TO_Q ( [ 0.0_dp, 0.0_dp, -1.0_dp] );Q(:,6)  = UV_TO_Q ( [ CN1,-CN2,0.5_dp] )
                Q(:,7)  = UV_TO_Q ( [ CN1,     CN2,    0.5_dp] );Q(:,8)  = UV_TO_Q ( [-CN1, CN2,0.5_dp] )
                Q(:,9)  = UV_TO_Q ( [ 0.0_dp, 0.0_dp, -1.0_dp] );Q(:,10) = UV_TO_Q ( [-CN1,-CN2,0.5_dp] )
                Q(:,11) = UV_TO_Q ( [ 0.0_dp, 0.0_dp, -1.0_dp] );Q(:,12) = UV_TO_Q ( [ CN1,-CN2,0.5_dp] )
                Q(:,13) = UV_TO_Q ( [ CN1,     CN2,    0.5_dp] );Q(:,14) = UV_TO_Q ( [-CN1, CN2,0.5_dp] )
                Q(:,15) = UV_TO_Q ( [ 0.0_dp, 0.0_dp, -1.0_dp] );Q(:,16) = UV_TO_Q ( [-CN1,-CN2,0.5_dp] )
                Q(:,17) = UV_TO_Q ( [ 0.0_dp, 0.0_dp, -1.0_dp] );Q(:,18) = UV_TO_Q ( [ CN1,-CN2,0.5_dp] )
                Q(:,19) = UV_TO_Q ( [ CN1,     CN2,    0.5_dp] );Q(:,20) = UV_TO_Q ( [-CN1, CN2,0.5_dp] )
                Q(:,21) = UV_TO_Q ( [ 0.0_dp, 0.0_dp, -1.0_dp] );Q(:,22) = UV_TO_Q ( [-CN1,-CN2,0.5_dp] )
                Q(:,23) = UV_TO_Q ( [ 0.0_dp, 0.0_dp, -1.0_dp] );Q(:,24) = UV_TO_Q ( [ CN1,-CN2,0.5_dp] )
            ENDIF
        ELSE
            A1             = 1.0766387466333334_dp
            A2             = 3.8985305527_dp
            A3             = 7.0052176543_dp
            IDEAL_CELL_VLM = A1*A2*A3
            SCALE          = (CELL_VLM/IDEAL_CELL_VLM)**(1.0_dp/3.0_dp)
            ...
            BOX = [ A1*REAL(NUCX,DP), A2*REAL(NUCY,DP), A3*REAL(NUCZ,DP) ]
            PRINT *, "FDDD UNIT CELL:", BOX
            R(:,1)  = [A1*0.125_dp, A2*0.125_dp, A3*0.625_dp];R(:,2)  = [A1*0.125_dp, A2*0.375_dp, A3*0.875_dp]
            R(:,3)  = [A1*0.000_dp, A2*0.500_dp, A3*0.000_dp];R(:,4)  = [A1*0.375_dp, A2*0.375_dp, A3*0.625_dp]
            R(:,5)  = [A1*0.250_dp, A2*0.250_dp, A3*0.750_dp];R(:,6)  = [A1*0.375_dp, A2*0.125_dp, A3*0.875_dp]
            R(:,7)  = [A1*0.125_dp, A2*0.625_dp, A3*0.125_dp];R(:,8)  = [A1*0.125_dp, A2*0.875_dp, A3*0.375_dp]
            R(:,9)  = [A1*0.000_dp, A2*0.000_dp, A3*0.500_dp];R(:,10) = [A1*0.375_dp, A2*0.875_dp, A3*0.125_dp]
            R(:,11) = [A1*0.250_dp, A2*0.750_dp, A3*0.250_dp];R(:,12) = [A1*0.375_dp, A2*0.625_dp, A3*0.375_dp]
            R(:,13) = [A1*0.625_dp, A2*0.125_dp, A3*0.125_dp];R(:,14) = [A1*0.625_dp, A2*0.375_dp, A3*0.375_dp]
            R(:,15) = [A1*0.500_dp, A2*0.500_dp, A3*0.500_dp];R(:,16) = [A1*0.875_dp, A2*0.375_dp, A3*0.125_dp]
            R(:,17) = [A1*0.750_dp, A2*0.250_dp, A3*0.250_dp];R(:,18) = [A1*0.875_dp, A2*0.125_dp, A3*0.375_dp]
            R(:,19) = [A1*0.625_dp, A2*0.625_dp, A3*0.625_dp];R(:,20) = [A1*0.625_dp, A2*0.875_dp, A3*0.875_dp]
            R(:,21) = [A1*0.500_dp, A2*0.000_dp, A3*0.000_dp];R(:,22) = [A1*0.875_dp, A2*0.875_dp, A3*0.625_dp]
            R(:,23) = [A1*0.750_dp, A2*0.750_dp, A3*0.750_dp];R(:,24) = [A1*0.875_dp, A2*0.625_dp, A3*0.875_dp]
            IF(RIGIDT) THEN
                CN1 = 0.23053648_dp
                CN2 = 0.83477713_dp
                CN3 = 0.5_dp
    ...
    ELSEIF(NSITES==3) THEN
        ...
        ELSE
            CELL_VLM = PI*16.0_dp / (6.0_dp * RHO)
        ENDIF
    !   Ideal density = 0.5721936308
        A1             = 1.44543719034967_dp
        A2             = 3.18963740723535_dp
        A3             = 6.06507858870017_dp
        IDEAL_CELL_VLM = A1*A2*A3
        SCALE          = (CELL_VLM/IDEAL_CELL_VLM)**(1.0_dp/3.0_dp)
        A1             = A1*SCALE
        A2             = A2*SCALE
        ...
        BOX = [ A1*REAL(NUCX,DP), A2*REAL(NUCY,DP), A3*REAL(NUCZ,DP) ]
        PRINT *, "Trigonal FDDD UNIT CELL:", BOX

        R(:,1)  = [A1*0.50000, A2*0.50000, A3*0.08334_dp]
        R(:,2)  = [A1*0.50000, A2*0.50000, A3*0.91666_dp]
        R(:,3)  = [A1*0.25000, A2*0.75000, A3*0.83334_dp]
        R(:,4)  = [A1*0.75000, A2*0.75000, A3*0.16666_dp]
        R(:,5)  = [A1*0.50000, A2*0.00000, A3*0.58334_dp]
        R(:,6)  = [A1*0.50000, A2*0.00000, A3*0.41666_dp]
        R(:,7)  = [A1*0.25000, A2*0.25000, A3*0.33334_dp]
        R(:,8)  = [A1*0.75000, A2*0.25000, A3*0.66666_dp]
        R(:,9)  = [A1*0.00000, A2*0.50000, A3*0.58334_dp]
        R(:,10) = [A1*0.00000, A2*0.50000, A3*0.41666_dp]
        R(:,11) = [A1*0.75000, A2*0.75000, A3*0.33334_dp]
        R(:,12) = [A1*0.25000, A2*0.75000, A3*0.66666_dp]
        R(:,13) = [A1*0.00000, A2*0.00000, A3*0.08334_dp]
        R(:,14) = [A1*0.00000, A2*0.00000, A3*0.91666_dp]
        R(:,15) = [A1*0.75000, A2*0.25000, A3*0.83334_dp]
        R(:,16) = [A1*0.25000, A2*0.25000, A3*0.16666_dp]
        IF(RIGIDT) THEN
            Q(:,1)  = [  0.18148240969544657_dp,-0.10266502684412926_dp,-0.48471469974144310_dp, 0.84945611251481379_dp]
            Q(:,2)  = [ -0.97520838955431266_dp, 0.00644680360414193_dp,-0.00831167920222054_dp,-0.22103834883343645_dp]
            Q(:,3)  = [  0.18793928650722080_dp,-0.10773952034255306_dp, 0.49142884956162514_dp,-0.84354531956619749_dp]
            Q(:,4)  = [  0.49250336183217930_dp,-0.84549985620479784_dp, 0.18064422682076414_dp,-0.09968999477370009_dp]
            Q(:,5)  = [ -0.84908364407327275_dp,-0.48647109807787486_dp, 0.11177231721930644_dp, 0.17294445699583477_dp]
            Q(:,6)  = [ -0.47757988738443413_dp,-0.85397815564056856_dp,-0.17324192265113630_dp,-0.11236546217924880_dp]
            Q(:,7)  = [  0.00424111129038517_dp, 0.22275192254746021_dp,-0.97480923574755807_dp,-0.01051417508343410_dp]
            Q(:,8)  = [  0.11438721848050000_dp, 0.19555177511473104_dp, 0.84230157477848877_dp, 0.48908396479887584_dp]
            Q(:,9)  = [  0.84773635437844319_dp,-0.48778189498413960_dp, 0.10524053112622020_dp,-0.17982304357095610_dp]
            Q(:,10) = [  0.97665843824766163_dp,-0.00028398426754942_dp,-0.01477242867312568_dp, 0.21428949975135872_dp]
            Q(:,11) = [  0.18609335151292458_dp, 0.09585419745742993_dp,-0.50082111876605295_dp,-0.83985679990722339_dp]
            Q(:,12) = [  0.97850528071497356_dp, 0.00124014113251931_dp, 0.00445435469867987_dp,-0.20616992115022439_dp]
            Q(:,13) = [ -0.00426711751209170_dp, 0.97531592293062608_dp,-0.22018249523627434_dp, 0.01613415562229568_dp]
            Q(:,14) = [ -0.21611683727286587_dp, 0.00447504923907879_dp, 0.00105390493042277_dp, 0.97635668475505866_dp]
            Q(:,15) = [  0.17704320126546841_dp,-0.11384085131886140_dp, 0.49205685837946622_dp,-0.84473428578364895_dp]
            Q(:,16) = [ -0.48939462289920993_dp, 0.84804483874510106_dp,-0.17212131994829882_dp, 0.10810691825425195_dp]
        ENDIF
    ENDIF
...
SUBROUTINE OCTAHEDRAL_SC(UCL)
    ...
    C1 = ( 1.0_dp+SQRT(2.0_dp) ) / 2.0_dp * 1.5_dp
    C2 = ( 1.0_dp+2.0_dp*SQRT(2.0_dp) ) / 2.0_dp * 1.5_dp
    C3 = 0.5_dp*1.5_dp
